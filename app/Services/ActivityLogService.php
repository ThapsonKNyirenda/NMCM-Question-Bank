...
use App\Contracts\ActivityLog as ActivityContract;
...
class ActivityLogService
{
    ...
    protected ?string $defaultLogName  = 'default';

    protected ?ActivityContract $activity = null;

    protected LogBatch $batch;

    public function __construct(LogBatch $batch)
    {
        $this->batch = $batch;
    }

    /**
     * Set the model on which the activity was carried on
     *
     * @param Model $model
     * @return $this
     */
    ...
    public function causedBy(Model | int | string | null $modelOrId ): static
    {
        ...
        if (is_numeric($modelOrId) || is_string($modelOrId)){
            $model = User::findOrFail($modelOrId);
        }else{
            $model = $modelOrId;
        }
